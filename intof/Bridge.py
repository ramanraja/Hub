...
def extract_status (message):  
    ...
    devid = sp[1] 
    rsid  = sp[2]    
    if (rsid.startswith('POWER')):
        dprint ("Parsed: ", sp, payload)
    elif (rsid.startswith('RESULT')):
        pass # redundant status 
    else:
        dprint ("Parsed: ", sp)  # sensor & network data create a lot of dumping on the consile
...
# see also:  operate_offline (), which can be used offline or from backend for the same.
...
def on_socket_event (payload):
    ...
    try:
        ...
        #TODO: if the device is offline, queue it in LKG status:
        ## if (device-is-offline):
        ###  operate_offline (device_id, relsen_id, action)
    except Exception as e:
...
@app.route('/operate/offline', methods=['GET', 'POST'])
def operate_offline_route():
    try:
        if (request.method=='GET'):
            jcmd = {
                'device_id' : request.args.get('device_id'),
            ...
            if (request.json is None):
                return ({'result' : False, 'error':'invalid device'})
            jcmd = request.json  
        return operate_offline (jcmd['device_id'], jcmd['relsen_id'], jcmd['action'])
    except Exception as e:
        print ('* EXCEPTION 8: ', str(e))
    return ({'result' : False, 'error' : 'could not enqueue the command'}) 
                
                
# Somewhat the same effect as set_relay_status(), but this can be called : 
#    (1) by the user, especially when the device is offline (2) called internally from scheduler-timer events
# The command is just queued in last_known_good status and executed whenever the device comes online
# Note: this can be used both to swith ON and switch OFF the device. (and, may be toggle?)
def operate_offline (device_id, relsen_id, action):   
    global last_good_status
    dprint ("Operating offline: {}/{}:{}".format (device_id,relsen_id,action))
    if last_good_status is None:
        last_good_status = {}
    if not device_id in last_good_status:   # create the key
        last_good_status[device_id] = {}    # NOTE: now you can operate even devices that are not yet registered ***
    last_good_status [device_id][relsen_id] = action      
    return ({'result' : True, 'msg' : 'offline command put in queue'}) 
